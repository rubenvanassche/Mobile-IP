...
void MobileIPNode::run_timer(Timer *timer) {
    ...
    Timestamp now = Timestamp::now_steady();
		...
		if(this->connection.lifetime != 65535 and this->connection.remainingLifetime > 0){
				this->connection.remainingLifetime -= 1;

				if(this->connection.remainingLifetime <= ((double)this->connection.lifetime/2.0)){
					if(this->connection.isHome == false and this->requests.has(this->connection.agentAddress, this->connection.careOfAddress) == false){
